#############
# Joint test for Hardy-Weinberg proportions in males and females --- Chromosome 22 _t15
#############
# 6/20/2017

# qsub -N hwe_Chr22_t15 -m e -M wlhsu@uw.edu -q bigmem.q /projects/QCpipeline/runRscript.sh /projects/wlhsu/src/hwe/jointHW/hwe_joint_Chr22_t15.R
...
fileName    <- "Chr22.geno.v01.wlh.RData"
fileIn      <- paste(machinePath,filePath,fileName,sep="/")
snp         <- get(load(fileIn)); dim(snp)
#head(snp,n=2)
head(names(snp), 82)
...
asian <- snp[snp$hwe.asian.mcd,]; dim(asian)  # 352

# remove SNP with all NA with variables "scanID" and "sex"
rNA.asi <- cbind(asian$scanID,asian$sex,Filter(function(x) !all(is.na(x)), asian[,80:ncol(asian)])); dim(rNA.asi)  # 352
head(names(rNA.asi))
...
saveas(rNA.asi, "Chr22.geno.v02.wlh.RData", "/projects/wlhsu/results/hwe/HardyWeinberg/asian/Autosome")

# check how many males and femals
table(rNA.asi$sex)

# (1) get those 221 males only
asian.M <- rNA.asi[rNA.asi$sex %in% "M",]; dim(asian.M)
M <- asian.M[,3:ncol(asian.M)]; dim(M)
head(M,2)

# (2) get those 131 females only
asian.F <- rNA.asi[rNA.asi$sex %in% "F",]; dim(asian.F)
F <- asian.F[,3:ncol(asian.F)]; dim(F)
head(F,2)
...
cM <- data.frame(snpID, C); dim(cM)
head(cM)

saveas(cM, "Chr22.M.count.v02.wlh.RData", "/projects/wlhsu/results/hwe/HardyWeinberg/asian/Autosome")
...
snpID = colnames(F)
colnames(C) <- c("nAA", "nAB", "nBB", "nNA")
cF <- data.frame(snpID, C); dim(cF)
head(cF)

saveas(cF, "Chr22.F.count.v02.wlh.RData", "/projects/wlhsu/results/hwe/HardyWeinberg/asian/Autosome")
...
saveas(asian.Ch, "Chr22.jmf.t15.pv.wlh.RData", "/projects/wlhsu/results/hwe/HardyWeinberg/asian/Autosome")
...
rownames(meta)[idx]    
meta["exact.pv.asian.jmf.ch22", "labelDescription"] <- "p-value from joint exact test with truncate at 10^-15 for Hardy-Weinberg proportions in males and females on chromosome 22 in set of 352 hwe.asian.mcd=TRUE samples"
meta["midp.pv.asian.jmf.ch22", "labelDescription"] <- "p-value from joint mid-p test with truncate at 10^-15 for Hardy-Weinberg proportions in males and females on chromosome 22 in set of 352 hwe.asian.mcd=TRUE samples"
...
head(pData(snp))
saveas(snp, "snp.annot.v04.t15.ch22.wlh.RData", "/projects/wlhsu/results/hwe/HardyWeinberg/asian/Autosome")
